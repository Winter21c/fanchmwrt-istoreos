...
推上去之后会自动编译，成功后创建一个同名 Release 并把镜像挂上去。
（这条路径用的是默认参数：含 Docker、192.168.1.1、1GB 分区。）

**方式三：每天凌晨 2 点自动构建**

工作流里带了一个定时任务，每天自动编译一份并发布，参数固定为：
...
| rootfs 分区 | 2048 MB |
| Docker | 含 |
| 发布标签 | `daily`（**滚动**，每天替换同一个 Release） |

两个要点：

- **cron 是 UTC**。北京时间的凌晨 2 点对应 `0 18 * * *`（前一天 18:00 UTC），
  写成 `0 2 * * *` 会在北京时间上午 10 点触发。
- **用滚动标签 `daily` 而不是每天新建一个 Release**：否则一年积压 365 个
  Release、每个几百 MB，releases 页面没法看。想保留每日历史，把
  「确定 Release 标签」里的 `daily` 换成 `daily-$(date -u +%Y%m%d)` 即可。

> ⚠️ GitHub 会在仓库**连续 60 天没有任何提交活动**后自动停用定时工作流，
