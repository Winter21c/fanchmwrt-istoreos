...
工作流加了第三个触发方式：`schedule`。参数固定为
**192.168.100.1 / 2048 MB / 含 Docker**，发布标签 `daily`。

### 14.1 cron 是 UTC，不是本地时间

这是最容易踩的一条：**GitHub Actions 的 cron 一律按 UTC 解释**，
与仓库、runner 的时区都无关。

想要北京时间凌晨 2 点，就得写 `0 18 * * *`（前一天 18:00 UTC）。
写 `0 2 * * *` 会在北京时间**上午 10 点**触发 —— 差 8 小时，
而且这种错误只会在第二天才被发现。

### 14.2 为什么用滚动标签

定时构建固定发到 `daily` 这个标签，也就是每天**替换**同一个 Release。

若改成每天新建（比如 `daily-20260925`），一年就是 365 个 Release、
每个几百 MB —— releases 页面基本没法用了。滚动标签保证永远只有一个
「最新每日构建」。工作流里已有的「更新 Release 时清掉过期附件」逻辑
正好配套：附件列表始终与当次构建一致。

要保留每日历史的话，把「确定 Release 标签」里的 `daily` 换成
`daily-$(date -u +%Y%m%d)` 即可。

### 14.3 参数是怎么传进定时构建的

定时触发时 `inputs.*` 全是空的，如果沿用原来那套 `env: LAN_IP: ${{ inputs.lan_ip }}`，
就会拿到空值、退回脚本默认（192.168.1.1 / 1024），与预期不符。

所以把这三个参数从 job 级 `env` 挪进了一个独立的「确定构建参数」步骤，
按事件类型分支后写进 `$GITHUB_ENV`：

```bash
if [ "${GITHUB_EVENT_NAME}" = "schedule" ]; then
    LAN_IP_VALUE="192.168.100.1"; ROOTFS_VALUE="2048"; DOCKER_VALUE="1"
else
    LAN_IP_VALUE="${{ inputs.lan_ip }}"; ...
fi
{ echo "LAN_IP=$LAN_IP_VALUE"; ... } >> "$GITHUB_ENV"
```

顺带避开一个坑：**写进 `$GITHUB_ENV` 的值要到下一个 step 才生效**，
在同一个 step 里 `echo $LAN_IP` 只会拿到空值。所以日志里打印的是
上面那几个局部变量，不是环境变量。

### 14.4 任务名也要跟着分支

任务名（运行列表里那一列）同样要区分事件类型：

```yaml
name: >-
  编译 x86_64 · LAN=${{
    github.event_name == 'schedule' && '192.168.100.1' || inputs.lan_ip || '192.168.1.1'
  }} · ...
```

不加 `event == 'schedule' && ...` 这一层的话，定时构建会显示成
「LAN=192.168.1.1 / 1024MB / Docker=true」，与实际编出来的东西相反 ——
比不显示还糟。
